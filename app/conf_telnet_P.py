...
def initial(node, conf_file):
    ...
    for node in conf_file["P_routers"]:
        if node["hostname"] == name:
            ...
            tn.write(b"network " + node["LinkNetwork1"].encode('utf-8') + b" " + node["LinkNetworkNetmask"].encode(
                'utf-8') + b" area " + conf_file["OSPF_config"][0]["OSPFareaCore"].encode('utf-8') + b"\r\n")
            tn.write(b"network " + node["LinkNetwork2"].encode('utf-8') + b" " + node["LinkNetworkNetmask"].encode(
                'utf-8') + b" area " + conf_file["OSPF_config"][0]["OSPFareaCore"].encode('utf-8') + b"\r\n")
